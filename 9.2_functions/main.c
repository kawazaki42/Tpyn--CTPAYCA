...
bool in_area(size_t i, size_t j, size_t order) {
    if (i == j)
        // _На_ главной диагонали
        return true;

    if (order-1 - i == j)
        // _На_ побочной диагонали
        return true;
    ...
    // Элемент входит в область, если
    // он _над_ или _под_ обеими диагоналями.
    // Элемент _не_ входит, если он _над_ ровно одной.
}
...
int main() {
    ...
    matrix_of_float m = read_matrix_of_float(mat_order, mat_order);
